...
import { Image, Eye, AlignLeft, Plus, UserLock, ChevronLeft, ChevronRight, Loader2 } from "lucide-react";
...
export const CreateCourseModal = ({
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      dispatch({ type: 'courses/addCourse', payload: newCourse }); // Make sure this is in your reducer
  ...
  return (
    <>
      ...
      <button
        type="button"
        className="flex items-center gap-2 card px-4 py-2 rounded-full transition"
      ...
      <AnimatePresence>
        {isModalOpen && (
          <>
            {/* Modal content */}
            <motion.div
              className="fixed z-[99] md:top-0 left-0 bottom-0 h-[100dvh] w-full mx-auto flex-center"
              initial={{ y: "100%" }}
              animate={{ y: 0 }}
              exit={{ y: "100%" }}
